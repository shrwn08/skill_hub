...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-light/95 backdrop-blur-md border-b border-soil/10">
      <div className="flex justify-between items-center px-4 sm:px-8 lg:px-16 py-4">

        {/* Logo */}
        <NavLink to="/" className="font-fraunces text-xl font-black text-soil">
          Entre<span className="text-amber">Skill</span> Hub
        </NavLink>

        {/* Hamburger for mobile */}
        <button
          className="lg:hidden flex items-center p-2 text-soil"
        ...
        <ul className={`flex flex-col lg:flex-row gap-2 lg:gap-4 list-none lg:items-center absolute lg:static top-full left-0 w-full lg:w-auto bg-light/95 lg:bg-transparent transition-all duration-300 overflow-hidden ${isOpen ? "max-h-96 py-4" : "max-h-0 lg:max-h-full"}`}>
          ...
          <li>
            <NavLink
              to="/signup"
              className="bg-soil text-cream px-4 py-1 rounded-full opacity-100 block text-center lg:inline-block"
              onClick={() => setIsOpen(false)}
            >
              Get Started
            </NavLink>
